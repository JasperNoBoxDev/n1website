(() => {
  ...
  const person = people.find((item) => slug(item.alias) === requested) || people[0];
  if (!person) return;
  ...
  document.title = `${person.alias} reports — n1.care`;
  document.querySelector('[data-profile-hero]').innerHTML = `<div><div class="report-profile-identity"><span class="report-profile-avatar" aria-hidden="true">${person.initials}</span><div><span class="page-kicker">Anonymized profile · Age range ${person.age}</span><h1>${person.alias}</h1></div></div><p>${person.context}</p><div class="report-profile-topics">${person.focus.map((area) => `<span>${area}</span>`).join('')}</div></div><div class="report-profile-meta"><b>${person.reports.length}</b><span>reports available</span></div>`;
